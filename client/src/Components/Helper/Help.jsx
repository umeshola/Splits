...
import { done_checking, get_all_help } from '../../connection/query';

export default function Help() {
    const { data: d1, refetch: r1 } = useQuery(get_all_help);
    const [flag, setFlag] = useState(false);
    const [handleDone] = useMutation(done_checking)
    useEffect(() => {
        if (d1?.get_help.length === 0) {
            setFlag(true);
        } else {
            setFlag(false);
        }
                    ...
                    {d1?.get_help.map((item, index) => (
                        ...
                        >
                            <div className='border-[1px] w-[250px] mx-auto mt-[10px] border-neutral-700 px-1 py-3 rounded-2xl bg-neutral-700 bg-opacity-45'>
                                <div>
                                    <h1 className='text-[20px]'>{item.on.platform} - {item.on.plan}</h1>
                                </div>
                            </div>
                            <div className='mt-5 ml-7'>
                                <div>
                                    ...
                                    <div>
                                        <h1 className='text-[13px] text-neutral-400 -mt-1'>{item.by.userName}</h1>
                                    </div>
                                </div>
                                <div className='my-1'>
                                    <div>
                                    ...
                                    <div>
                                        <h1 className='text-[13px] text-neutral-400 -mt-1'>{item.by.email}</h1>
                                    </div>
                                </div>
                                <div className='my-1'>
                                    <div>
                                    ...
                                    <div>
                                        <h1 className='text-[13px] text-neutral-400 -mt-1'>{item.by.phone}</h1>
                                    </div>
                                </div>
                            </div>
                            <div className='flex mt-[30px] justify-center border-[1px] rounded-lg mx-7 border-neutral-800 bg-neutral-700'>
                                <button onClick={() => fun(item.id)} // Update to use item.on.id
